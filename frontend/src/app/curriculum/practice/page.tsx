...
const typeLabel: Record<string, { label: string; icon: string }> = {
  SPEECH: { label: "Speech Prompt", icon: "\U0001f3a4" },
  DRAFT: { label: "Draft Resolution", icon: "\U0001f4dc" },
  NEGOTIATION: { label: "Negotiation", icon: "\U0001f91d" },
  QUIZ: { label: "Quiz", icon: "\u2753" },
  OPEN: { label: "Open-Ended", icon: "\U0001f4ac" },
};
      ...
      <section className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 pt-28 pb-12">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="flex items-center gap-3 mb-3">
            <span className="text-4xl">{"\U0001f3af"}</span>
            <div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex flex-wrap gap-3 mb-8 items-center">
          ...
          <button onClick={handleGenerate} disabled={generating}
            ...
            {generating ? "{'\U0001f989'} Thinking..." : "{'\u2728'} Generate AI Questions"}
          </button>
        </div>
        ...
        {aiQuestions.length > 0 && (
          <div className="mb-10">
            <div className="flex items-center gap-2 mb-4">
              <span className="text-2xl">{"\U0001f989"}</span>
              <div>
              ...
              {aiQuestions.map((q: any, i: number) => (
                <div key={i} className="bg-orange-50 rounded-xl p-5 border border-orange-200">
                  <div className="flex items-center gap-2 mb-2">
                    <span>{typeLabel[q.question_type]?.icon || "{'\U0001f4ac'}"}</span>
                  ...
                  {q.hints && (<p className="text-[10px] text-[#C66810] bg-orange-100/50 p-1.5 rounded mb-3">{"\U0001f4a1"} {q.hints}</p>)}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-slate-50 rounded-xl border border-slate-200">
            <div className="text-4xl mb-3">{"\U0001f989"}</div>
            ...
            {questions.map((q: any) => {
              ...
              return (
                <div key={q.id} className="bg-white rounded-xl p-5 border border-slate-200 hover:shadow-md hover:border-orange-200 transition-all">
                  ...
                  {q.hints && (<p className="text-[10px] text-[#C66810] bg-orange-50 p-1.5 rounded mb-3">{"\U0001f4a1"} {q.hints}</p>)}
...
export default function PracticePage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-4xl">{"\U0001f989"}</div></div>}>
      <PracticeContent />
    </Suspense>
  );
}
